...
describe('LabelingForm', () => {
  ...
  it('should populate form with existing annotation', () => {
    render(<LabelingForm video={mockVideo} existingAnnotation={mockAnnotation} />);
  ...
  it('should show error when submitting without labels', async () => {
    ...
    expect(screen.getByText('Please select labels for both speakers')).toBeInTheDocument();
  ...
  it('should show error when submission fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Failed to save annotation. Please try again.')).toBeInTheDocument();
    });
  });
  ...
  it('should update confidence when slider changes', async () => {
    ...
    const confidenceSliders = screen.getAllByRole('slider').filter(
      slider => slider.getAttribute('min') === '1' && slider.getAttribute('max') === '5'
    );
  ...
  it('should render metadata if present', () => {
    ...
    render(<LabelingForm video={videoWithMetadata} existingAnnotation={null} />);
